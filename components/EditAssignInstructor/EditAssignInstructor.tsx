...
import { assignPackage, getAssignByStdId, editAssignInstructor } from 'services/room'
...
const EditAssignInstructor = ({ params }: any) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await editAssignInstructor(data, params.Id)
        console.log('Instructor assigned api response', res)
        toast.success('Instructor update successfully', {
          position: 'top-right',
          autoClose: 5000,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getAssignByStdId(params.Id)
        console.log('Data isiefnaliemfoaienl:', res)
        const data1 = res.AssignData
        setInstructorId(data1.instructor_id._id)
        setStudentId(data1.std_id._id)
        let existanceData = res.AssignData
        existanceData.no_of_lesson = data1.no_of_lesson.toString()

        existanceData.road_test = data1.road_test
        existanceData.studentName = `${data1.std_id.firstName} ${data1.std_id.lastName}`
        existanceData.instructorName = `${data1.instructor_id.firstName} ${data1.instructor_id.lastName}`
        console.log('The existance data is :', existanceData)
        setData(existanceData)
      } catch (error) {
...
export default EditAssignInstructor
